...
public sealed class MouseListener : IDisposable
{
    ...
    {
        _subscriptions.Add(new MouseSubscription(mouseEvent, onAction, intervalOfClick ?? TimeSpan.Zero));
    }
    
    public void SubscribeOnce(MouseEvent mouseEvent, Action onAction)
    {
        _subscriptions.Add(new MouseSubscription(mouseEvent, onAction, true));
    }
